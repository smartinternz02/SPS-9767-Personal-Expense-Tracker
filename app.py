...
app.config['MYSQL_USER'] = '5imjwzu9Dr'
app.config['MYSQL_PASSWORD'] = '1sGeQBTRd4'
app.config['MYSQL_DB'] = '5imjwzu9Dr'
mysql = MySQL(app)
            ...
            if session['s_m']:
                ...
                if (total['tsum']>bud):
                    message = Mail(
                        ...
                        html_content='<h1>PERSONAL EXPENSE TRACKER</h1> <h3> Dear'+' '+ session['username'] + '</h3> <p style="color:red"> You have exceeded your monthly budget of amount'+' '+str(session['budget'])+ ', For the month of'+' '+session['s_m']+'.</p><br>You current expenses are worth:'+session['total']+'<p>Yours Truely,<br>ABC</p>')
                try:
                    sg = SendGridAPIClient('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')
                    response = sg.send(message)
                    print(response.status_code)
                    print(response.body)
                    print(response.headers)
